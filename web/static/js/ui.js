$(function(){
	...
	$('.main .btn_up').click(function(e){
		...
		var $html = $('html, body'),
			...
			$speed = 700;
		if($(this).hasClass('on')){
			$html.animate({'scrollTop':($btnTop-$height+30)},$speed);
		}else{
	...
	$('.main .mainPopList a').click(function(e){
		e.preventDefault();
		$(this).closest('.mainPopWrap').toggleClass('on').siblings('.mainPopWrap').removeClass('on');;
		$(this).closest('li').siblings().find('.mainPopWrap').removeClass('on');
	});	
});
function layoutUi(){
...
function radarChartInit(){
	...
	var gradientColor = ctx.createLinearGradient(170,0, 410,340);
	...
	var labelAr = [
		...
		['여드름 균', '피부결', '', '', '', '', '',''],
		['여드름 균', '피부결', '광채,브라운색소', '', '', '', '',''],
		['여드름 균', '피부결', '광채,브라운색소', '모공', '', '', '',''],
		['여드름 균', '피부결', '광채,브라운색소', '모공', '홍조,멜라닌,피지', '', '',''],
		['여드름 균', '피부결', '광채,브라운색소', '모공', '홍조,멜라닌,피지', '피부밝기', '',''],
		['여드름 균', '피부결', '광채,브라운색소', '모공', '홍조,멜라닌,피지', '피부밝기', '다크써클',''],
		['여드름 균', '피부결', '광채,브라운색소', '모공', '홍조,멜라닌,피지', '피부밝기', '다크써클','미래주름']
	];
	var datasNum = 0;
